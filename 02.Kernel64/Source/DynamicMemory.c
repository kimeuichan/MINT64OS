...
static QWORD kCalculateDynamicMemorySize(void){
	...
	// 3GB 이상의 메모리에는 비디오 메모리와 프로세서가 사용하는 레지스터가 존재하므로 최대 3GB 까지 사용 가능
...
static int kCalculateMetaBlockCount(QWORD qwDynamicRAMSize){
	...
	// 가장 크기가 작은 블록의 개수를 계산하여 이를 기준으로 비트맵 영역과 할당된 크기를 저장하는 영역을 계산
...
void* kAllocateMemory(QWORD qwSize){
	...
	// 버디 블록 할당하고 할당된 블록이 속한 블록 리스트의 인덱스를 반한
...
static int kFindFreeBlockInBitmap(int iBlockListIndex){
	...
	for(i=0; i<iMaxCount;){
		// QWORD 8*8=>64비트이므로, 64비트를 한번에 검사해서 1인 비트가 있는지 확인
		if( ((iMaxCount - i)/64) > 0){
...
BOOL kFreeMemory(void* pvAddress){
	...
	// 넘겨 받은 어드레스를 블록 풀을 기준으로 하는 어드레스로 변환하여 할당했던 블록 크기를 검색
	...
	// 할당된 블록이 속한 블록 리스트의 인덱스가 저장된 곳을 초기화하고, 할당된 블록이 포함된 블록 리스트를 검색
...
static BOOL kFreeBuddyBlock(int iBlockListIndex, int iBlockOffset){
	...
	// 블록 리스트의 끝까지 인접한 블록을 검사하여 결합할 수 없을 때 까지 반복
	for(i=iBlockListIndex; i<gs_stDynamicMemory.iMaxLevelCount; i++){
		// 현재 블록은 존재하는 상태로 설정
		kSetFlagInBitmap(i, iBlockOffset, DYNAMICMEMORY_EXIST);

		// 블록의 오프셋이 짝수면 홀수를 검사, 홀수면 짝수의 비트맵을 검사하여 인접한 블록이 존재한다면 결합
		if( (iBlockOffset % 2) ==0)
