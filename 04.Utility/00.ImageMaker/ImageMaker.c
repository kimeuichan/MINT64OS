...
int main(int argc, char * argv[]){
	...
	if( (iSourceFd = open(argv[2], O_RDONLY)) == -1){
		fprintf(stderr, "[ERROR] %s open fails!\n",argv[2]);
		exit(-1);
	}
	...
	// 파일 크기를 섹터 크기인 512 바이트로 맞추기 위해 나머지 부분을 0x00으로로 채움
	...
	WriteKernelInformation(iTargetFd, iKernel32SectorCount+iKernel64SectorCount, iKernel64SectorCount);
...
void WriteKernelInformation(int iTargetFd, int iKernelSectorCount, int iKernel32SectorCount){
	...
	//파일 시작에서 5바이트 떨어진 위치가 커널의 총 섹터 수 정보를 나타냄
	...
	// 부트 로더를 제외한 총 섹터 수 및 보호 모드 커널의 섹터수 저장
